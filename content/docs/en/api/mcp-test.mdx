...
| `auth`            | -                    | Send bearer credentials to a URL-connected server - see [testing OAuth-protected servers](/en/docs/api/auth). Throws on a stdio or in-process server, which cannot send one. |
